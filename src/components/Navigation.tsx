    ...
    <nav className="bg-white shadow-sm fixed bottom-0 left-0 right-0 md:relative">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between md:justify-start gap-4 py-4">
          <NavButton icon={<Home />} label="Tasks" onClick={() => setActiveView('tasks')} />
          <NavButton icon={<Users />} label="Members" onClick={() => setActiveView('members')} />
          <NavButton icon={<Settings />} label="Settings" onClick={() => setActiveView('settings')} />
          <NavButton
            icon={<PlusCircle />}
            label="New Task"
