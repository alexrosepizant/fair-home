...
import { getTaskStressLevel, suggestTaskReassignment } from '../utils/taskDistribution';
...
export function TaskCard({ task }: Props) {
  ...
  const handleReassignment = () => {
    const newAssignees = suggestTaskReassignment(task, users, task.assignedTo[0]);
  ...
  return (
    <>
      ...
      {showFeedback && (
        <EmotionalFeedbackModal task={task} onClose={() => setShowFeedback(false)} />
      )}
    </>
  );
}